...
contract Election is Ownable, ReentrancyGuard {
     ...
     */
    constructor(string memory _name) Ownable(msg.sender) ReentrancyGuard() {
    ...
    /**
     * @notice Cast an anonymous vote backed by a zero-knowledge proof.
     *
     * @dev    Integration steps (to complete in next milestone):
     *           1. Deploy a Groth16 verifier generated by snarkjs from your Circom circuit.
     *           2. Store its address in `verifierAddress` (add a setter restricted to owner).
     *           3. Replace the placeholder `_verifyProof()` call with:
     *                IVerifier(verifierAddress).verifyProof(a, b, c, input)
     *              where the public inputs include the nullifier and candidateId.
     *
     * @param  _candidateId  Candidate being voted for.
     * @param  _nullifier    Unique hash derived from voter secret — prevents double voting
     *                       without revealing the voter's address.
     * @param  _zkProof      Encoded Groth16 proof bytes from the frontend (snarkjs).
     */
        ...
        inPhase(Phase.Voting)
    {
        ...
        require(candidates[_candidateId].exists, "Candidate does not exist");
        require(_verifyProof(_zkProof, _nullifier, _candidateId), "Invalid ZK proof");

        usedNullifiers[_nullifier] = true;
        candidates[_candidateId].voteCount++;
        totalVotes++;

        // Voter address is NOT stored or emitted — only the nullifier
        emit VoteCastWithProof(_nullifier, _candidateId);
    }

    /**
     * @dev Placeholder proof verifier.
     *      Replace with a real IVerifier interface call once the Circom
     *      circuit and snarkjs verifier contract are ready.
     */
    function _verifyProof(
        ...
        // TODO: call deployed Groth16 verifier contract
        return true;
    }
